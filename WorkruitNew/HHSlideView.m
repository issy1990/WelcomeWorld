...
#define Title_DefaultColor              [UIColor colorWithRed:62/255.0f green:187/255.0f blue:100/255.0f alpha:1.0f]
...
-(void)selectSelectedIndex:(int)index
{
    [self buttonClicked:[_buttonsArray objectAtIndex:index]];
    NSLog(@"%d",index);
}
...
- (void)addContentScrollView {
    ...
    for (UIViewController *vc in self.childControllersArray) {
        ...
        [vc.view setFrame:CGRectMake(index * self.frame.size.width, 0, self.frame.size.width, self.frame.size.height-SlideView_Height)];
...
- (UIButton *)p_customButtonWithFrame:(CGRect)rect forTitle:(NSString *)title withTag:(NSInteger)tag {
    ...
    tag == 0 ? [button.titleLabel setFont:[UIFont fontWithName:@"SourceSansPro-Semibold" size:16.0f]]: [button.titleLabel setFont:[UIFont fontWithName:@"SourceSansPro-Regular" size:16.0f]];
...
- (void)p_animateSliderToPositionWithOffset:(CGPoint)offset {
    ...
    if(flag != buttonTag){
            [self.delegate slideView:self didSelectItemAtIndex:buttonTag];
            flag = buttonTag;
        
            [self setButtonFont:buttonTag];
    }

    [[_buttonsArray objectAtIndex:buttonTag] setTitleColor:[_slider backgroundColor] forState:UIControlStateNormal];
}

-(void)setButtonFont:(int)tag
{
    for (UIButton *btn in _buttonsArray) {
            if(btn.tag == tag)
                btn.titleLabel.font = [UIFont fontWithName:@"SourceSansPro-Semibold" size:16.0f];
            else
                btn.titleLabel.font = [UIFont fontWithName:@"SourceSansPro-Regular" size:16.0f];
    }
}
...
- (void)buttonClicked:(UIButton *)button {
    ...
    //Setting font
    [self setButtonFont:(int)button.tag];
}
